...
**Add a map mutator (issue #54)** — a per-map environmental modifier rolled once per eligible map (level 11+) from the run seed. Add an entry to [public/mapMutators.yml](public/mapMutators.yml) (`id`, `name`, `description`, `clarify?`, `behavior`, optional `minLevel`/`maxLevel`/`weight`/`params`/`overtimePremium`). Reusing an existing `behavior` (`crunch`, `overclock`, `gravity`) needs no code. A brand-new behavior needs a case in [mapMutators.ts](src/lib/mapMutators.ts) `mutatorSpeedFactor`/`resolveMutator` and its application seam: ball/mover speed rides `game.creepFactor` (folded in [useGameLoop.ts](src/hooks/useGameLoop.ts)) and `updateMovers`; positional effects go in [updateBall.ts](src/lib/physics/updateBall.ts). A positional effect must be **legible**: the `conveyor` behaviour (a steady invisible current) was removed because a force acting on every ball all map with nothing on screen to show it reads as the map being broken, and this game's skill is predicting where a ball will be. Any `overtimePremium` is hazard pay: it rides outside the five axes via `flatBonus` in [applyCut.ts](src/lib/physics/applyCut.ts), so it is owed whatever route the run took. The chip + hold-to-clarify live in [GameTopBar.tsx](src/components/game/GameTopBar.tsx); the roll is a `useMemo` in [GameScreen.tsx](src/components/game/GameScreen.tsx). `noneWeight` leaves some maps vanilla.
...
**Add a map objective (issue #55)** — an optional, non-failing per-map goal rolled 0-or-1 per eligible map (level 11+) from the run seed, paying a bonus under the cap when met. Add an entry to [public/objectives.yml](public/objectives.yml) (`id`, `name`, `description`, `clarify?`, `kind`, optional `minLevel`/`maxLevel`/`weight`, `params`, `reward`). Reusing an existing `kind` (`lockCount`, `superiorLocks`, `underPar`, `speedClear`) needs no code, they are pure reads of existing per-map counters. A new `kind` needs a case in [mapObjectives.ts](src/lib/mapObjectives.ts) `evaluateObjective` plus, if it reads a counter not yet mirrored to React, a field on `GameStateInfo` (see the `superiorLocks` mirror in [GameCanvas.tsx](src/components/game/GameCanvas.tsx)). The reward is awarded at clear via `objectiveClearReward` folded into `extraBonus` in [applyCut.ts](src/lib/physics/applyCut.ts); the green chip + completion pulse + hold-to-clarify live in [GameTopBar.tsx](src/components/game/GameTopBar.tsx); the roll + live progress are `useMemo`s in [GameScreen.tsx](src/components/game/GameScreen.tsx). `noneWeight` leaves most maps objective-free.
