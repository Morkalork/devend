...
**Add a boss map (issue #56)** — give a level (10/20/30/40) a `boss` block ([types/level.ts](src/types/level.ts) `BossConfig`): `name`, `intro`, a mandatory `objective` (a #55 `MapObjective`), optional forced `mutator` (a #54 `MapMutator`), `creepFromStart`, and `phases`. The objective becomes a **hard win gate**: `isBossGateSatisfied` in [applyCut.ts](src/lib/physics/applyCut.ts) blocks `evaluateWinConditions` from completing until it is met (the deadline `timeLimit` is the fail state). GameScreen forces the boss objective/mutator instead of rolling and shows a one-time intro via `TutorialOverlay`. Phases fire once at a space/time threshold from [bossPhases.ts](src/lib/physics/bossPhases.ts) (`tickBossPhases`, wired into `spawnTimedBalls`), reusing `createBall` for adds. Winning level 10 flows into the existing Promotion capstone draft. Keep the objective always reachable (e.g. lock N of the *starting* balls) so a phase add can never soft-lock the win.
