...
const MODIFIER_META: Record<keyof GameModifiers, ModifierMeta> = {
  ...
  disablePushYourLuck:              { label: 'Disable Push Luck',      kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Bus Factor loadout: > 0 banks straight through, no push prompt' },
  ballFreezeDuration:               { label: 'Feature Freeze (s)',     kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Feature Freeze: seconds a tapped ball stays frozen (0 = off)' },
  freezeUsesPerMap:                 { label: 'Freeze Uses/Map',        kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Feature Freeze: tap-freezes allowed per map, refilled each map (0 = off)' },
  moverBrake:                       { label: 'Mover Brake',            kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Control Freak Junior: press and hold a mover to stop it where it is (0 = off)' },
  moverDrive:                       { label: 'Mover Drive Rate',       kind: 'additive',       step: 0.25, min: 0,    defaultValue: 0,    description: 'Control Freak Senior: drag a mover along its own rail, at up to this multiple of its patrol speed (0 = off)' },
  moverDerailPerMap:                { label: 'Mover Derails/Map',      kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Control Freak Principal A: derails per map, refilled each map. Hold a mover against its end stop to break it (0 = off)' },
  moverBandPerMap:                  { label: 'Mover Bumper Snaps/Map', kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Control Freak Principal B: bumper snaps per map, refilled each map. Pull a mover off rest and release to fling what it hits (0 = off)' },
  slowOneBallFactor:                { label: 'Slow One Ball (x)',      kind: 'additive',       step: 0.05, min: 0,    defaultValue: 0,    description: 'Runtime Opt B: speed multiplier for one random ball each map (0 = off, 0.5 = half speed)' },
  freezePickups:                    { label: 'Freeze Pickups',         kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Cryo Protocol: >0 = pickup tokens never expire (iced over until claimed)' },
  ...
  bugSquashChance:                  { label: 'Bug Squash (%)',         kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Bug Squash: percent chance a wall hit squashes the ball flat and sticks it (0 = off)' },
  bugSquashSeconds:                 { label: 'Bug Squash (s)',         kind: 'additive',       step: 0.5,  min: 0,    defaultValue: 0,    description: 'Bug Squash: seconds a squashed ball stays stuck to the wall' },
  extraContinues:                   { label: 'Extra Continues',        kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Extra per-run revives (runs start with none)' },
  showHighscoreProgress:            { label: 'Highscore Bar',          kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Benchmarking certificate: show the map-highscore progress bar in the HUD (>0 = on)' },
  ...
  overtimePerSuperiorLock:          { label: 'Overtime/Superior Lock', kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Severance Package (Equity Package): extra flat overtime per Superior Lock, on top of Overtime/Lock' },
  ...
  gravityLockBonus:                 { label: 'Gravity Lock Bonus',     kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Free Fall: extra lock-bonus multiplier for a ball locked under gravity, in a live well or on a gravity map (1 = double)' },
  gravityBendMultiplier:            { label: 'Gravity Bend',          kind: 'multiplicative', step: 0.05, min: 0,    defaultValue: 1,    description: 'Free Fall (Escape Velocity): how hard gravity bends your heading. Below 1 you fly straighter through wells and gravity maps' },
  ...
  fenceSpeedPerMapCleared:          { label: 'Fence Speed/Map Cleared', kind: 'additive',       step: 0.01, min: 0,    defaultValue: 0,    description: 'Warm Cache: fence-speed bonus per map cleared this run (0.10 = +10%; capped)' },
  ...
  winRequiresSplitLocks:            { label: 'Run: split-lock win',   kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Locks demanded on EACH side of every eligible map, on top of its own win' },
  bankedSlowPerStep:                 { label: 'Banked Slow/step',       kind: 'additive',       step: 0.01, min: 0,    defaultValue: 0,    description: 'War Chest: ball-speed reduction per 13h banked at map start (max 8%)' },
  ...
  overtimeCapBonus:                 { label: 'Overtime Cap +',         kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Stock Options capstone: raises the per-map overtime cap' },
  ...
  scopeCreepImmediate:              { label: 'Creep Immediate',        kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Hard Deadline rung: >0 = Scope Creep surges start at second 0 (no grace window)' },
  runwayInstantFenceAt:             { label: 'Runway Fence @h',        kind: 'additive',       step: 25,   min: 0,    defaultValue: 0,    description: 'Runway: bank threshold granting +1 instant fence per map while met (0 = off)' },
  runwayConcurrentFenceAt:          { label: 'Runway Concurrent @h',   kind: 'additive',       step: 25,   min: 0,    defaultValue: 0,    description: 'Runway: bank threshold granting +1 concurrent fence while met (0 = off)' },
  runwayFreezeAt:                   { label: 'Runway Freeze @h',       kind: 'additive',       step: 25,   min: 0,    defaultValue: 0,    description: 'Runway: bank threshold granting a 2s tap-freeze while met (0 = off)' },
  spendInstantFencePerChunk:        { label: 'Spend Fence/Chunk',      kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Budget Cycle: instant fences on the next map per 15h spent in one shop visit' },
  spendFenceSpeedPerChunk:          { label: 'Spend Fence Speed/Chunk',kind: 'additive',       step: 0.05, min: 0,    defaultValue: 0,    description: 'Budget Cycle: fence-speed bonus on the next map per 15h spent (0.05 = +5%)' },
  spendCapturePerChunk:             { label: 'Spend Capture/Chunk',    kind: 'additive',       step: 0.05, min: 0,    defaultValue: 0,    description: 'Budget Cycle (Retained Earnings): next-map board pre-capture per 15h spent (0.05 = +5%)' },
  spendChunkCapBonus:               { label: 'Spend Chunk Cap +',      kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Budget Cycle (Leveraged Buyout): raises the per-visit chunk ceiling above 3' },
  shipEarlyBonusMultiplier:         { label: 'Ship Early Mult.',       kind: 'multiplicative', step: 0.5,  min: 0.5,  defaultValue: 1,    description: 'Multiplies the Ship Early payout' },
  ...
  startingOvertime:                 { label: 'Starting Overtime',    kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Signing Bonus: overtime hours already banked when a run starts' },
  storeLockRelief:                  { label: 'Store Lock Relief',    kind: 'additive',       step: 1,    min: 0,    defaultValue: 0,    description: 'Corporate Card: 1 = shop never asks for more than one lock, 2 = it never asks at all' },
...
const PLAYGROUND_LEVEL: LevelConfig = {
  ...
  sizeThreshold: 5,
  expectedCuts: 10,
  points: 5,   // what every shipped map pays; was 100, stale since before the deflation
  balls: [
