/**
 * WHICH BALLS still count as a gate zone's target.
 *
 * Half of "can this zone still be reached": the other half is whether the board
 * has sealed it off, which lives in gateZoneReachable.test.ts. This one is the
 * roster, and it had drifted:
...
import { gateTargets } from "@/lib/coloredAreas";

const ball = (state: string, isBoss = false) => ({ state, isBoss });

/**
 * The predicate this file used to test directly.
 *
 * `anyGateTargetInPlay` was retired when the fail path stopped asking "is a
 * target alive" and started asking "can a target still get there" - a live ball
 * on the wrong side of a fence is not a ball that can reach the zone. Every
 * assertion below is about the ROSTER either question is asked over, which is
 * now one exported list, so they moved onto it rather than being deleted with
 * the wrapper.
 */
const anyGateTargetInPlay = (balls: { state: string; isBoss?: boolean }[]) =>
  gateTargets(balls).length > 0;
...
describe("the fail path uses it", () => {
  it("goes through the reachability check, which reads this roster", () => {
    ...
    expect(src, "the areaUnreachable check no longer calls the helper")
      .toContain("anyGateTargetCanReach(game.spaceGrid, game.balls");
    // And the old speed test is gone from that check, or the bug is only
    // half-removed.
    expect(src).not.toMatch(/b\.state !== "won" && b\.speed > 0/);
  });

  it("keeps the roster in ONE place, so the two questions cannot disagree", () => {
    const src = readFileSync(resolve(process.cwd(), "src/lib/coloredAreas.ts"), "utf8");
    expect(src).toMatch(/const targets = gateTargets\(balls\)/);
  });
});
