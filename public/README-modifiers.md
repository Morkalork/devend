...
| `bugSquashChance` | `0` | Percent chance (0..100) that a ball hitting a board edge, fence or static obstacle squashes flat against it and sticks (Bug Squash). Tiers sum (5+3+4 → 12%). A stuck ball rides the `frozenUntil` path: immovable, no trail, exempt from the speed floor, and Frozen Assets pays on it. Movers never stick a ball. Rolled from the run's seeded stream, so a Daily replays the same sticks. | `5` |
| `bugSquashSeconds` | `0` | Seconds a squashed ball stays stuck before it un-squashes and carries on (Bug Squash). Tiers sum (2+1+0.5 → 3.5s). No re-freeze cooldown is applied afterwards: that cooldown taxes taps, and this was never a tap. | `2` |
| `showHighscoreProgress` | `0` | `> 0` reveals the map-highscore HUD bar (Benchmarking certificate, unlocked by the Senior Dev achievement): a second bar under the capture readout tracking the live projected score against the map's stored highscore. Purely a HUD toggle, no gameplay effect. | `1` |
...
| `overtimeCapBonus` | `0` | Raises the per-map overtime backstop by this many hours (Stock Options capstone), reaching the scorer as `capRaise`. It pays nothing on a map that never presses against the backstop, which is most of them; it is what lets the last hours through on one that does. Until #79 it was passed as `flatBonus` and paid as flat hours instead, which is the opposite behaviour in both directions. | `5` |
